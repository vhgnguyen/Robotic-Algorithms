...
def calc_diff(target, x, y, yaw):
    d = np.array([target.x - x[-1],
                  target.y - y[-1],
                  bmm.pi_2_pi(target.yaw - yaw[-1])])

    return d


def calc_j(target, p, h, k0):
    xp, yp, yawp = bmm.generate_last_state(
        p[0, 0] + h[0], p[1, 0], p[2, 0], k0)
    dp = calc_diff(target, [xp], [yp], [yawp])
    xn, yn, yawn = bmm.generate_last_state(
        p[0, 0] - h[0], p[1, 0], p[2, 0], k0)
...
def selection_learning_param(dp, p, k0, target):
    ...
    for a in np.arange(mina, maxa, da):
        tp = p + a * dp
        xc, yc, yawc = bmm.generate_last_state(
            tp[0], tp[1], tp[2], k0)
...
def optimize_trajectory(target, k0, p):
    for i in range(max_iter):
        xc, yc, yawc = bmm.generate_trajectory(p[0], p[1], p[2], k0)
...
def test_optimize_trajectory():  # pragma: no cover
    ...
    #  target = bmm.State(x=5.0, y=2.0, yaw=np.deg2rad(00.0))
    target = bmm.State(x=5.0, y=2.0, yaw=np.deg2rad(90.0))
    k0 = 0.0

    init_p = np.array([4.0, 0.0, 0.0]).reshape(3, 1)
